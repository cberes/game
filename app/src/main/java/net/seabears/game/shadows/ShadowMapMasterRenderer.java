...
public class ShadowMapMasterRenderer implements Renderer {
     ...
     * This includes the {@link ShadowBox} which calculates the position and size of the
     * "view cuboid", the simple renderer and shader program that are used to render objects to the
     * shadow map, and the {@link ShadowFrameBuffer} to which the scene is rendered. The size of the
     * shadow map is determined here.
     * 
     * @param camera - the camera being used in the scene.
     */
    public ShadowMapMasterRenderer(Camera camera, ShadowShader shader, ShadowBox box, int displayWidth, int displayHeight) {
        this.shader = shader;
        this.shader.init();
        this.shadowBox = box;
     ...
     * rendering the main render pass. It converts a world space position into a 2D coordinate on
     * the shadow map. This is needed for the second part of shadow mapping.
     * 
     ...
     * "view cuboid" based on the information that was calculated in the {@link SHadowBox} class.
     * The light's "view" matrix is also calculated based on the light's direction and the center
     * position of the "view cuboid" which was also calculated in the {@link ShadowBox} class. These
     * two matrices are multiplied together to create the projection-view matrix. This matrix
     * determines the size, position, and orientation of the "view cuboid" in the world. This method
     * also binds the shadows FBO so that everything rendered after this gets rendered to the FBO.
     * It also enables depth testing, and clears any data that is in the FBOs depth attachment from
     * last frame. The simple shader program is also started.
     * 
     ...
     * position, so the "view" matrix is centered at the center of the "view cuboid". The created
     * view matrix determines where and how the "view cuboid" is positioned in the world. The size
     * of the view cuboid, however, is determined by the projection matrix.
     * 
    ...
    /**
     * Create the offset for part of the conversion to shadow map space. This conversion is
     * necessary to convert from one coordinate system to the coordinate system that we can use to
     * sample to shadow map.
     * 
